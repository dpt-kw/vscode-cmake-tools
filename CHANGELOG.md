...
Improvements:
- Add `cmake.exportSarifFile` setting to log diagnostics to a SARIF file when configuring CMake using the `CMAKE_EXPORT_SARIF` cache variable and `cmake.loadSarifFile` to make the extension read diagnostics from a generated SARIF file. Requires CMake 4.0 or newer. [#5069](https://github.com/microsoft/vscode-cmake-tools/issues/5069)
